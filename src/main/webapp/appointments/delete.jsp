...
<div class="container-fluid">
    <div class="row flex-nowrap">
        ...
        <div class="col py-3">
            ...
            <button type="button" class="btn btn-primary"><a href="/appointments?action=add">Thêm Lịch Khám</a></button>
            <table class="table">
                <thead>
                <tr>
                    <th>Mã Lịch Hẹn</th>
                ...
                <c:forEach var="item" items="${appointments}">
                    <tr>
                        <th scope="row">${item.id_appointment}</th>
                        <th>${item.appointment_date}</th>
                        <td>${item.status}</td>
                        <td>${item.patient.name}</td>
                        <td>${item.doctor.name}</td>
                        <td>
                            <a href="/appointments?action=edit&idEdit=${item.id_appointment}"><i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="/appointments?action=delete&idDelete=${item.id_appointment}"><i class="fa-solid fa-trash"></i></a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="deleteLabel">Xóa Lịch Khám</h5>
                            <a href="${pageContext.request.contextPath}/appointments?action=home" class="btn-close"></a>
                        </div>
                        <div class="modal-body">
                            <p>Bạn Có Chắc Muốn Xóa Lịch Khám
                                <strong><c:out value="${appointmentDelete != null ? appointmentDelete.id_appointment : '---'}"/></strong>?
                            </p>
                        </div>
                        <div class="modal-footer">
                            <a href="${pageContext.request.contextPath}/appointments?action=home" class="btn btn-secondary">Quay lại</a>

                            <form id="deleteForm" action="${pageContext.request.contextPath}/appointments?action=delete" method="post" style="display:inline;">
                                <input type="hidden" name="id_appointment" value="<c:out value='${appointmentDelete.id_appointment}'/>" />
